...
function UserAppointment(props) {
  ...
  props.status !== "cancelled" && timeLeft < 0
    ? (status = "passed")
    : (status = "pending");
  ...
  function setState(datepicker = true) {
    ...
    if (datepicker) setResponse(<Datepicker />);
      ...
      setResponse(
        <div>
          In cazul anularii programarilor, sumele achitate <b>nu</b> vor fi
          restituite! Puteti reprograma sedinta, dar nu mai tarziu de 7 zile de
          la data programata. Daca doriti sa continuati cu anularea sedintei,
          apasati pe butonul "Confirma"!"
        </div>
      );
  }
