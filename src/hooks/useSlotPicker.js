...
function useSlotPicker(props){
    ...
    useEffect(function() {
        fetch(`https://zknyo7t9m3.execute-api.eu-west-3.amazonaws.com/dev/slots/${currentMonth.firstDay}/${currentMonth.lastDay}`)
            .then(res => res.json())
            .then(data => {
              // response with all booked slots in database within the specified month
